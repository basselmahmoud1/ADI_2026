function data_out = sdf_stage_2(in_elem, clk, T) %#codegen
    DELAY_SIZE = 2;
    READ  = false;
    WRITE = true;
    ...
    delay_out = delay_x(cast(complex(0), 'like', T.reg2), DELAY_SIZE, READ, T);

    if counter < 2
        % Cycles 0, 1: Orange Path
        % 1. Multiply the delayed data by twiddle factor -> output it
        % 2. Store the incoming data into the delay line
        twiddle_idx = (counter * 2) + 1;
